...
마지막 질문이, 여러분이 문제를 풀기 위하여 알고리즘을 짜야하는 부분입니다. 알고리즘은 어떠한 특정한 문제를 풀기위한 레시피라고 생각하시면 됩니다. 알고리즘은 특정한 절차를 의사코드로 작성을 해둔 것을 의미합니다.

#### 의사코드(Pesudo Code)

의사코드란 여러분의 프로그램의 논리 혹은 절차를 코드 대신 우리가 쓰는 언어로 적은 것을 의미합니다. 의사코드는 여러분이 문제를 풀기 위하여 밟아야 되는 절차를 조금 더 천천히 명확하게 볼 수 있게 도와줍니다. 

예시를 한 번 볼게요. 유저에게 어떠한 숫자를 받아서, 0부터 그 숫자까지의 모든 번호를 출력하는 프로그램을 의사코드로 적어보겠습니다

> 유저에게 숫자를 인풋으로 받는다
횟수를 셀 count 변수를 선언하고 값을 0으로 둔다
count 변수가 유저가 입력한 인풋보다 작다면 count 변수의
값을 출력한다. 
그리고 1만큼 값을 더 올린다.
유저가 입력한 숫자와 일치할때까지 반복한다.
